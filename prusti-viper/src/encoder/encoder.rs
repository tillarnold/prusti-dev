...
use super::snapshot_encoder::SnapshotAdtEncoder;
...
impl<'v, 'tcx> Encoder<'v, 'tcx> {
    ...
    pub fn encode_stub_pure_function_use(
        ...
        // only optimized_mir.
        //let body = self.env.tcx().optimized_mir(proc_def_id); // FIXME: https://github.com/viperproject/prusti-dev/issues/286
    ...
    fn fold_field(
        ...
        match field_name.as_str() {
            ...
            _ => {
                ...
                let domain_name = self
                    .find_possible_domain_for_field_name(&field_name)
                    .expect(&format!("No doman found with a field named {}", field_name));
