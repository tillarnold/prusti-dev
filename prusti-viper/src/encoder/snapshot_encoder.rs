...
pub struct SnapshotAdtEncoder<'s, 'p: 's, 'v: 'p, 'tcx: 'v> {
...
impl<'p, 'v, 'r: 'v, 'a: 'r, 'tcx: 'a> SnapshotEncoder<'p, 'v, 'tcx> {
    ...
    fn encode_primitive(&self, field: vir::Field) -> Snapshot {
        Snapshot {
            predicate_name: self.predicate_name.clone(),
    ...
    fn encode_generic(&self) -> PositionlessEncodingResult<Snapshot> {
    ...
    fn encode_tuple(&self) -> PositionlessEncodingResult<Snapshot> {
        let snap_domain = self.encode_snap_domain()?;
        Ok(Snapshot {
            predicate_name: self.predicate_name.clone(),
    ...
    fn encode_cons_func_args(
        ...
        subst: ty::subst::SubstsRef<'tcx>,
    ) -> PositionlessEncodingResult<Vec<vir::Expr>>
    {
        let tcx = self.encoder.env().tcx();
        variant
            .fields
    ...
    fn encode_field_funcs(&self) -> Option<(Vec<vir::DomainFunc>, Vec<String>)> {
        if self.adt_def.is_struct() {
            ...
            for field in self.adt_def.all_fields() {
                funcs.push(self.encode_field_domain_func(field).unwrap());
                fields.push(field.ident.name.to_ident_string());
            }
